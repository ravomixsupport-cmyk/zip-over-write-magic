...
const STORAGE_KEY_PREFIX = "ravomix_complaint_form_";

const ComplaintForm = () => {
  const { id } = useParams();
  const template = complaintTemplates.find((t) => t.id === id);
  const storageKey = `${STORAGE_KEY_PREFIX}${id}`;
  const { t, lang } = useAppLang();
  const isHi = lang === 'hi';
  const seoTitle = template ? `${template.title} — Free Format & Sample | Ravomix` : "Complaint Letter — Ravomix";
  useSEO({
    title: seoTitle.length > 70 ? `${template?.title ?? "Complaint"} | Ravomix` : seoTitle,
    description: template
      ? `Write a ${template.title.toLowerCase()} in minutes. Fill the form, preview, download or share the PDF. Free template in English & Hindi.`
      : "Generate complaint letters instantly with Ravomix.",
    path: `/complaints/${id ?? ""}`,
    ogType: "article",
  });
    ...
    <Layout>
      <div className="px-4 pt-4 md:px-6 lg:px-8">
        <div className="no-print mb-4 flex items-center gap-3">
          <Link to="/complaints" className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl bg-card shadow-sm border border-border">
